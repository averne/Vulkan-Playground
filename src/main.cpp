...
int main(int argc, char **argv) {
    ...
    auto *infp = std::fopen("scripts/in.bin", "rb");
    SCOPEGUARD([&infp] { std::fclose(infp); });
    if (!infp) {
    ...
    struct stat st;
    if (auto rc = ::fstat(::fileno(infp), &st); rc < 0) {
        std::println("Failed to tell file size: {}", std::strerror(errno));
        return 1;
    }

    auto *flagfp = std::fopen("scripts/flag.bin", "rb");
    SCOPEGUARD([&flagfp] { std::fclose(flagfp); });

    auto array_size = st.st_size;
    auto inbuf_size  = array_size * sizeof(std::uint32_t),
        flagbuf_size = array_size * sizeof(std::uint8_t),
    ...
    auto flagbuf = VK_CHECK_RV(dev->createBufferUnique(vk::BufferCreateInfo(vk::BufferCreateFlags(), inbuf_size,
                                                                           vk::BufferUsageFlagBits::eShaderDeviceAddress |
                                                                           vk::BufferUsageFlagBits::eStorageBuffer       |
                                                                           vk::BufferUsageFlagBits::eTransferDst)));
    ...
    auto flagmem = VK_CHECK_RV(dev->allocateMemoryUnique(vk::MemoryAllocateInfo(dev->getBufferMemoryRequirements(*inbuf ).size,
                                                                               host_buf_idx, &memalloc_pnext)));
    ...
    VK_CHECK(dev->bindBufferMemory(*flagbuf, *flagmem, 0));
    ...
    auto flagshdaddr = dev->getBufferAddress(vk::BufferDeviceAddressInfo(*flagbuf));
    ...
    auto *inaddr   = VK_CHECK_RV(dev->mapMemory(*inmem,  0, inbuf_size));
    auto *flagaddr = VK_CHECK_RV(dev->mapMemory(*flagmem, 0, flagbuf_size));
    SCOPEGUARD([&] { dev->unmapMemory(*inmem);   });
    SCOPEGUARD([&] { dev->unmapMemory(*flagmem); });

    if (auto rd = std::fread(inaddr, sizeof(std::uint8_t), array_size, infp); rd != array_size) {
        std::println("Failed to read input file: {} (got {}, expected {})", std::strerror(errno), rd, array_size);
        return 1;
    }

    if (auto rd = std::fread(flagaddr, sizeof(std::uint8_t), array_size, flagfp); rd != array_size) {
        std::println("Failed to read flag file: {} (got {}, expected {})", std::strerror(errno), rd, array_size);
        return 1;
    }

    VK_CHECK(dev->flushMappedMemoryRanges({vk::MappedMemoryRange(*inmem,   0, inbuf_size),
                                           vk::MappedMemoryRange(*flagmem, 0, flagbuf_size), }));
    ...
    cmdbuf.pushConstants<PushData>(*pipeline_layout, vk::ShaderStageFlagBits::eCompute, 0,
        PushData(inshdaddr, flagshdaddr, outshdaddr, tmpshdaddr, 0, array_size));
    ...
    if (auto wr = std::fwrite(inaddr, sizeof(uint8_t), outbuf_size, fpout); wr != outbuf_size) {
